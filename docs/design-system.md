...
| `GalleryLink.astro`  | home               | The link at the foot of the home page. **On scroll, not on click** — at 60% visible, seventeen sketched birds burst up and away to the left like a startled flock. X and Y are animated on separate elements with different easing, which bends the path into an arc rather than a straight diagonal; each bird banks slowly, beats its wings deeply and slightly out of time with its neighbours, and leaves in a ragged wave rather than all at once. Decorative and `aria-hidden`; `pointer-events: none`, the link navigates normally, nothing happens under reduced motion, re-arms when the section leaves the viewport. |
| `FilmStrip.astro`    | home (via StoryBand) | A length of 35mm film: dark base, perforations down both edges as a repeating gradient rather than forty elements, orange edge codes, and a thin frame line around the exposure. Fades slowly between the photographs a story names. Two strips on a page are staggered so they never change at the same moment. Holds on the first frame under reduced motion. |
